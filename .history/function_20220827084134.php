...
if(isset($_POST['outbound'])) {
    ...
    $esen = mysqli_query($conn, "delete serial_number from stock where serial_number = ".intval($_POST['outboud'])" ");
        if($esen !== $serial_number){
            
        }
        $laptopout = mysqli_query($conn, "insert into keluar (serial_number, status) SELECT serial_number, status FROM stock where serial_number = $sn, delete from stock");
