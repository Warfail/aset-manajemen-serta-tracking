...
if(isset($_POST['outbound'])) {
    $hapus = $_POST['outbound'];

  
    $sn = mysqli_query($conn, "select serial_number from stock where");
    $laptopout = mysqli_query($conn, "insert into keluar (serial_number, status) SELECT serial_number, status FROM stock");
    $esen = mysqli_query($conn, "delete serial_number from stock where serial_number = '$sn' ");
    if($laptopout !== $serial_number){
    }
    if($esen !== false) {
        
        echo '<script>alert("status di konfirmasi"); </script>';
    } else {
