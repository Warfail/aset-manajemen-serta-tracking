...
if(isset($_POST['outbound'])) {
    ...
    $esen = mysqli_query($conn, "select serial_number from stock where serial_number = '$serial_number' ");
        ...
        $laptopout = mysqli_query($conn, "insert into keluar (serial_number, status) SELECT serial_number, tujuan FROM stock");
    
    if($laptopout ) {
        echo '<script>alert("data berhasil di mas"); </script>';
    } else {
