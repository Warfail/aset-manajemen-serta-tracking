...
if(isset($_POST['laptopkeluar'])) {
    ...
    $addmasuk = mysqli_query($conn, "insert into keluar(serial_number, brand, keterangan) values ('$serial_number', '$brand','$keterangan')");
    $updatestockmasuk = mysqli_query($conn, "delete from stock where serial_number = '$serial_number' stock(serial_number, brand, model, processor, ram, storage) values ('$serial_number', '$brand','$model', '$processor', '$ram', '$storage')");
    if($addmasuk && $updatestockmasuk ) {
        header('location:masuk.php');
    } else {
