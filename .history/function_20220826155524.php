...
if(isset($_POST['laptopkeluar'])) {
    $hapuslap = $_POST['laptopkeluar'];
    $serial_number = $_POST['serial_number'];
    // $tujuan = $_POST['tujuan'];
    
   
    $ceksn = mysqli_query($conn, "select serial_number from stock where serial_number ='$hapuslap' ");
    while ($sn = mysqli_fetch_array($ceksn)){
        ...
        // $sn = mysqli_query($conn, "delete serial_number from stock where ");
    ...
    $addkeluar = mysqli_query($conn, "insert into keluar (serial_number, tujuan ) from serial_number * from stock ");
    if($addkeluar && $ceksn) {
        header('location:keluar.php');
    } else {
