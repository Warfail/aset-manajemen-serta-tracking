...
if(isset($_POST['outbound'])) {
    $se = $_POST['serial_number'];
    $status = $_POST['status'];
  
    $takesn = mysqli_query($conn, "select * from stock where serial_number = '$takesn' ");
    $esen = mysqli_query($conn, "delete serial_number from stock where serial_number = '$takesn' ");
        if($esen !== $serial_number){
            
        }
        $laptopout = mysqli_query($conn, "insert into keluar (serial_number, status) SELECT serial_number, status FROM stock where serial_number = $sn");
