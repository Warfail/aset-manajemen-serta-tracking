...
if(isset($_POST['outbound'])) {
    ...
    $sn = mysqli_query($conn, "select serial_number from stock where serial_number = '$serial_number' ");
    $esen = mysqli_query($conn, "delete serial_number from stock where serial_number = '$sn' ");
        if($esen !== $serial_number){
            
        }
        $laptopout = mysqli_query($conn, "insert into keluar (serial_number, status) SELECT serial_number, status FROM stock where serial_number = $serial_number");
