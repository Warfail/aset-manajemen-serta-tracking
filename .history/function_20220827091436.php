...
if(isset($_POST['outbound'])) {
    ...
    $esen = mysqli_query($conn, "select serial_number from stock where serial_number = '$outbound' ");
        if($esen !== $sn){
    ...
    } $delete = mysqli_query($conn, "delete from stock where serial_number = '$sn'");
    if($delete !== false) {
        
        echo 'gagal';
        header('location:masuk.php');
    } else {
        echo '<script>alert("proses gaga"); </script>';
    }
