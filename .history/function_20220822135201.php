...
if(isset($_POST['laptopkeluar'])){
    ...
    while ($rs = mysqli_fetch_array($cekrusak)){
        ...
        $serial_number = $_POST['serial_number'];
        $tujuan = $_POST['tujuan'];
        $keluarlapi = mysqli_query($conn, "insert into keluar(serial_number,tujuan) values ('$serial_number','$tujuan')");
        $hapusstock = mysqli_query($conn, "delete from rusak where serial_number ='$serial_number'");
        if($addkeluar && $hapusstock){
            header('location:keluar.php');
        } else {
            echo 'gagal';
