...
if(isset($_POST['laptopkeluar'])) {
    ...
    // $addkeluar = mysqli_query($conn, "insert into keluar ( tujuan ) values ( '$tujuan') ");
    $klar = mysqli_query($conn, "insert into keluar (serial_number ) select serial_number from stock ");
    if($klar !== $ceksn) {
        cho '<script>alert("data tidak ada") </script>';
    } else {
