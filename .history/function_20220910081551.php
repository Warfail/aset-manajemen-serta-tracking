...
if(isset($_POST['laptopmasuk'])){
    ...
    $liatstok = mysqli_query($conn, "select serial_number from stock");
    $addmasuk = mysqli_query($conn, "insert into masuk(serial_number, brand, keterangan) values ('$serial_number', '$brand','$keterangan')");
    $updatestockmasuk = mysqli_query($conn, "insert into stock(serial_number, brand, model, processor, ram, storage ,keterangan) values ('$serial_number', '$brand','$model', '$processor', '$ram', '$storage', '$keterangan')");
    if($addmasuk && $updatestockmasuk !==$liatstok ) {
...
if(isset($_POST['outbound'])) {
    ...
    $ram = $_POST['ram'];
    $storage = $_POST['storage'];
    $keterangan = $_POST['ketera'];
    $brand = $_POST['brand'];
    $model = $_POST['model'];
    
        $laptopout = mysqli_query($conn, "insert into keluar (brand, model, serial_number, ram, storage, keterangan ) values ('$brand', '$model', '$serial_number', '$ram', '$storage', '$keterangan')");
        $laptopdelete =mysqli_query($conn, "delete from stock where serial_number = '$serial_number'");
    if($laptopout && $laptopdelete) {
...
if(isset($_POST['retur'])) {
    ...
    $status = $_POST['status'];
        ...
        $laptopretur = mysqli_query($conn, "insert into keluar (serial_number, ram, storage, status ) values ('$serial_number', '$ram', '$storage', '$status')");
        $hapusrusak =mysqli_query($conn, "delete from rusak where serial_number = '$serial_number'");
    if($laptopretur && $hapusrusak) {
        echo '<script>alert("data berhasil di masukan"); </script>';
    }  else {
        echo '<script>alert("gagal"); </script>';
    }
...
if(isset($_POST['laptoprusak'])){
    ...
    $addmasuk = mysqli_query($conn, "insert into masuk(serial_number, brand, keterangan) values ('$serial_number', '$brand','$keterangan')");
    $stokrusak = mysqli_query($conn, "insert into rusak(serial_number, brand, model, processor, ram, storage, keterangan) values ('$serial_number', '$brand','$model', '$processor', '$ram', '$storage', '$keterangan' )");
