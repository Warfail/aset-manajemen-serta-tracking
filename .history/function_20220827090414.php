...
if(isset($_POST['outbound'])) {
    ...
    $laptopout = mysqli_query($conn, "insert into keluar (serial_number, status) SELECT serial_number, status FROM stock where serial_number = '$hapus'");
    $esen = mysqli_query($conn, "delete from stock where serial_number = '$hapus' ");
    if($laptopout && $esen){
        echo '<script>alert("status di konfirmasi"); </script>';
    } else {
        echo '<script>alert("gaga"); </script>';
    }
