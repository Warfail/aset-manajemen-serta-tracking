...
if(isset($_POST['outbound'])) {
    $snhapus = $_POST['putbound'];
  

        $esen = mysqli_query($conn, "delete from stock where serial_number = '$snhapus' ");
        $laptopout = mysqli_query($conn, "insert into keluar (serial_number, status) SELECT serial_number, status FROM stock where serial_number = '$snhapus'");
