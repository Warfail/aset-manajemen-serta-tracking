...
if(isset($_POST['laptopkeluar'])) {
    ...
    $esen = mysqli_query($conn, "select serial_number from stock where serial_number = '$serial_number' ");
        if($esen !== $serial_number){
            echo '<script>alert("data tidak ada"); </script>';
        }
        $laptopout = mysqli_query($conn, "insert into keluar(serial_number, tujuan) values ('$serial_number', '$tujuan' )");
        $updateout = mysqli_query($conn, "delete from stock where serial_number = '$serial_number' ");  
    
    if($laptopout && $updateout ) {
        header('location:masuk.php');
    } else {
...
if(isset($_POST['outbound'])){
    ...
    $addmas = mysqli_query($conn, "insert into masuk(serial_number, brand, keterangan) values ('$serial_number', '$brand','$keterangan')");
