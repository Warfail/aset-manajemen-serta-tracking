...
if(isset($_POST['laptopkeluar'])){
    ...
    $hapusstock = mysqli_query($conn, "delete from stock where serial_number = $serial_(serial_number, brand, model, spesifikasi) values ('$serial_number', '$brand','$model', '$spesifikasi')");
    if($addmasuk && $updatestockmasuk){
        header('location:masuk.php');
    } else {
        echo 'gagal';
